...
def process_next_chunk(context, file, previous_chunk, imported_objects, CONSTRAIN_BOUNDS, IMAGE_SEARCH, WORLD_MATRIX, KEYFRAME):
    ...
    while (previous_chunk.bytes_read < previous_chunk.length):
        ...
        # another object is being processed
            ...
            for keydata in keyframe_data.items():
                child.location = mathutils.Vector(keydata[1]) * (CONSTRAIN_BOUNDS * 0.1) if object_id == ROOT_OBJECT and CONSTRAIN_BOUNDS != 0.0 else keydata[1]
            ...
            for keydata in keyframe_data.items():
                child.scale = mathutils.Vector(keydata[1]) * (CONSTRAIN_BOUNDS * 0.1) if object_id == ROOT_OBJECT and CONSTRAIN_BOUNDS != 0.0 else keydata[1]
